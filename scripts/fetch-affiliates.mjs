...
if (FEEDS.length === 0) {
  ...
  console.error('Kopiera config.example till .env.local och uppdatera med dina feed URLs');
  process.exit(1);
}
...
function generateMatchKeys(productName) {
  ...
  // Hantera series-nummer (t.ex. "wh 1000xm5" -> "wh1000xm5")
  keys.push(normalized.replace(/(\w+)\s+(\d+)/g, '$1$2'));
  
  // Hantera bindestreck mellan siffror
...
function findBestMatch(productName, candidates) {
    ...
    for (const productKey of productKeys) {
      for (const candidateKey of candidateKeys) {
        if (productKey === candidateKey) {
          return { candidate, matched_on: `exact:${productKey}`, confidence: 1.0 };
        }
      }
    }
  }
  
  // Contains match (min 8 tecken)
  for (const candidate of candidates) {
    const candidateKeys = generateMatchKeys(candidate.name || candidate.title);
    ...
    for (const productKey of productKeys) {
      for (const candidateKey of candidateKeys) {
        if (productKey.length >= 8 && candidateKey.length >= 8) {
          if (productKey.includes(candidateKey) || candidateKey.includes(productKey)) {
            const commonLength = Math.min(productKey.length, candidateKey.length);
            const confidence = commonLength / Math.max(productKey.length, candidateKey.length);
            return { candidate, matched_on: `contains:${candidateKey}`, confidence };
          }
        }
      }
    }
  ...
  // Levenshtein fallback (max 2 skillnader)
  let bestMatch = null;
      ...
      for (const candidateKey of candidateKeys) {
        if (productKey.length >= 8 && candidateKey.length >= 8) {
          ...
          if (distance <= 2 && distance < bestDistance) {
            ...
            bestMatch = { candidate, matched_on: `levenshtein:${candidateKey}`, confidence: 1 - (distance / Math.max(productKey.length, candidateKey.length)) };
          }
        }
      }
    }
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const response = await fetch(url, {
        headers: {
          'User-Agent': 'Prylval-Adtraction-Integration/1.0'
        }
      });
      ...
      console.log(`✅ Hämtade ${data.length || 'okänt antal'} produkter från ${url}`);
      return data;
...
async function fetchAllFeeds() {
  const feedPromises = FEEDS.map(async (feed) => {
    try {
      ...
      // Filtrera produkter utan image_url eller tracking_url
      ...
      console.log(`📊 ${feed.name}: ${validProducts.length}/${products.length} produkter har giltiga URLs`);
...
function loadEditorialProducts() {
  const editorialPath = join(__dirname, '..', 'data', 'editorial-products.json');
  ...
  // Flatta ut alla kategorier till en lista
  ...
  console.log(`📚 Laddade ${products.length} redaktionella produkter från ${Object.keys(editorialData).filter(k => !k.startsWith('_')).length} kategorier`);
...
function loadExistingAffiliateMap() {
  ...
  if (existsSync(mapPath)) {
    try {
      ...
      console.log(`📋 Laddade befintlig affiliate-map med ${Object.keys(existing).filter(k => !k.startsWith('_')).length} produkter`);
      return existing;
    } catch (error) {
      console.warn(`⚠️ Kunde inte läsa befintlig affiliate-map: ${error.message}`);
    }
  }
...
function matchProducts(editorialProducts, feeds) {
  ...
  // Sortera feeds efter prioritet (högre nummer = högre prioritet)
  ...
  for (const product of editorialProducts) {
    ...
    // Samla alla kandidater från alla feeds
    const allCandidates = [];
    for (const feed of sortedFeeds) {
      allCandidates.push(...feed.products.map(p => ({ ...p, merchant: feed.merchant, priority: feed.priority })));
    }
    ...
    if (match && match.confidence > 0.7) {
      ...
      // Kontrollera om vi redan har en bättre match (högre prioritet)
      if (!existingEntry || 
        ...
        // Varning om dubblett
        if (existingEntry && existingEntry.merchant !== match.candidate.merchant) {
          warnings.push(`⚠️ ${product.name}: Ersatte ${existingEntry.merchant} med ${match.candidate.merchant} (högre prioritet)`);
        }
      } else {
        // Behåll befintlig entry
      ...
      // Behåll befintlig entry om den finns
      if (affiliateMap[product.editor_key]) {
        matches[product.editor_key] = affiliateMap[product.editor_key];
        coverage[category].matched++;
        if (!coverage[category].merchants[affiliateMap[product.editor_key].merchant]) {
          coverage[category].merchants[affiliateMap[product.editor_key].merchant] = 0;
        }
...
function saveAffiliateMap(affiliateMap) {
  const mapPath = join(__dirname, '..', 'data', 'affiliate-map.json');
...
async function main() {
  try {
    ...
    // Matcha produkter
    console.log('\n🔍 Matchar produkter...');
    const { matches, coverage, warnings } = matchProducts(editorialProducts, feeds);
